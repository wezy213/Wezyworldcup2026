...
function fmtDate(d){
  return new Date(d).toLocaleString("ar-SA",{calendar:"gregory",dateStyle:"medium",timeStyle:"short"});
}
...
function usedDoubleCount(userId,excludeMatchId=null){
  return predictions.filter(x=>
    x.user_id===userId &&
    x.double_used &&
    (excludeMatchId===null || String(x.match_id)!==String(excludeMatchId))
  ).length;
}

function doubleCardHtml(m,p,l){
  if(!session)return "";
  const used=usedDoubleCount(session.user.id,m.id);
  const already=!!p?.double_used;
  const remaining=Math.max(0,3-used-(already?1:0));
  const disabled=l || (!already && used>=3);
  const note=already ? "تم استخدام الكرت لهذه المباراة" : `المتبقي: ${Math.max(0,3-used)} من 3`;
  return `
    <label class="double-card ${already?'active':''} ${disabled?'disabled':''}">
      <input id="d_${m.id}" type="checkbox" ${already?'checked':''} ${disabled?'disabled':''}>
      <span>🃏 استخدام كرت دبل النقاط</span>
      <small>${note}</small>
    </label>`;
}
function renderMatches(){
  ...
  $("matchesList").innerHTML=list.map(m=>{
    ...
    return `<div class="match ${l?'locked':''}">
      ...
      <div class="small match-note ${r?'computed':''}">
        ${r?'🟢 تم احتساب النقاط':(l?'🔒 التوقع مغلق':'⏳ التوقع مفتوح')}
        ${!r&&!l?`— يغلق بعد ${timeUntilLock(m)}`:''}
        ${p?`— توقعك: ${p.home_goals}-${p.away_goals}${p.double_used?' 🃏 دبل':''} | نقاط: ${pointsForPrediction(p)}`:''}
      </div>
    </div>`;
  }).join("");
}
...
async function savePrediction(id){
  ...
  const double_used=$("d_"+id)?.checked||false;
  const used=usedDoubleCount(session.user.id,id);

  if(double_used && used>=3){
    return alert("لقد استخدمت جميع كروت الدبل الثلاثة");
  }
  ...
  alert(double_used?"تم حفظ التوقع مع كرت الدبل 🃏":"تم حفظ التوقع");
  await loadAll();
}
...
function renderLeaderboard(){
  ...
  let prev={}; 
  try{prev=JSON.parse(localStorage.getItem('wezyRanks')||'{}')}catch(e){}

  const medals=["🥇","🥈","🥉"];
  $("leaderboardBody").innerHTML=rows.map((p,i)=>{
    const old=prev[p.id], move=old?old-(i+1):0;
    const moveTxt=move>0?` <span class="move up">⬆️ ${move}</span>`:move<0?` <span class="move down">⬇️ ${Math.abs(move)}</span>`:"";
    return `<tr class="${i<3?`top top-${i+1}`:""}"><td class="player-name">${medals[i]||""} ${p.username}${moveTxt}</td><td class="champion-pick">${team(p.champion_code).flag} ${team(p.champion_code).name||"-"}</td><td class="points-cell"><b>${p.pts}</b></td></tr>`;
  }).join("");

  const now={}; 
  rows.forEach((p,i)=>now[p.id]=i+1); 
  localStorage.setItem('wezyRanks',JSON.stringify(now));
}
...
function renderQuickSummary(){
  ...
  if(!prof){
    el.innerHTML=`<div><b>سجل دخولك</b><span>لتظهر نقاطك</span></div><div><b>المتصدر</b><span>${leader?leader.username+' — '+leader.pts:'-'}</span></div><div><b>الإقصائية المفتوحة</b><span>${matches.filter(m=>isKnockout(m)&&!locked(m)).length}</span></div>`;
    return;
  }

  const me=rows.find(p=>p.id===prof.id);
  const resultedIds=new Set(results.map(r=>String(r.match_id)));
  const myPreds=predictions.filter(p=>p.user_id===prof.id);
  const scored=myPreds.filter(p=>resultedIds.has(String(p.match_id))&&pointsForPrediction(p)>0).length;
  const done=myPreds.filter(p=>resultedIds.has(String(p.match_id))).length;
  const doublesLeft=Math.max(0,3-usedDoubleCount(prof.id));

  el.innerHTML=`<div><b>مركزك</b><span>#${rows.findIndex(p=>p.id===prof.id)+1}</span></div><div><b>نقاطك</b><span>${me?.pts||0}</span></div><div><b>توقعات صحيحة</b><span>${scored} من ${done}</span></div><div><b>كروت الدبل</b><span>${doublesLeft} من 3</span></div><div><b>المتصدر</b><span>${leader?leader.username+' — '+leader.pts:'-'}</span></div><div><b>بطلك</b><span>${team(prof.champion_code).flag} ${team(prof.champion_code).name||'-'}</span></div>`;
}
...
function renderMyResults(){
  ...
  if(!session){
    el.innerHTML=`<p class="muted">سجل دخولك حتى تشوف توقعاتك ونقاط كل مباراة.</p>`;
    return;
  }

  const mine=predictions
    .filter(p=>p.user_id===session.user.id)
    .sort((a,b)=>(matches.find(m=>m.id===a.match_id)?.match_no||0)-(matches.find(m=>m.id===b.match_id)?.match_no||0));

  if(!mine.length){
    el.innerHTML=`<p class="muted">ما عندك توقعات محفوظة حتى الآن.</p>`;
    return;
  }
  ...
  el.innerHTML=mine.map(p=>{
    const m=matches.find(x=>String(x.id)===String(p.match_id));
    const r=results.find(x=>String(x.match_id)===String(p.match_id));
    const pts=pointsForPrediction(p);
    let extra="";
    if(m&&isKnockout(m)&&Number(p.home_goals)===Number(p.away_goals)&&p.winner_code&&p.winner_code!=="DRAW") extra=` — المتأهل: ${label(p.winner_code)}`;
    if(p.double_used) extra+=` — 🃏 دبل`;
    return `<div class="result-card ${r?'done':''}"><b>مباراة ${m?.match_no||''}: ${label(m?.home_code,m?.home_placeholder)} × ${label(m?.away_code,m?.away_placeholder)}</b><span>توقعك: ${p.home_goals}-${p.away_goals}${extra}</span><span>النتيجة: ${r?`${r.home_goals}-${r.away_goals}`:'لم تُدخل بعد'}</span><strong>+${pts} نقاط</strong></div>`;
  }).join('');
}
function renderAccuracy(){
  const el=$("accuracyList"); 
  if(!el)return; 

  const resultIds=new Set(results.map(r=>String(r.match_id)));
  const rows=profiles.map(prof=>{
    const ps=predictions.filter(p=>p.user_id===prof.id&&resultIds.has(String(p.match_id)));
    const hits=ps.filter(p=>pointsForPrediction(p)>0).length;
    const exact=ps.filter(p=>{
      const r=results.find(x=>String(x.match_id)===String(p.match_id)); 
      return r&&Number(p.home_goals)===Number(r.home_goals)&&Number(p.away_goals)===Number(r.away_goals);
    }).length; 
    return {...prof,total:ps.length,hits,exact,pct:ps.length?Math.round(hits/ps.length*100):0};
  }).filter(x=>x.total>0).sort((a,b)=>b.pct-a.pct||b.exact-a.exact||b.hits-a.hits).slice(0,10);

  if(!rows.length){
    el.innerHTML=`<p class="muted">تظهر الإحصائية بعد إدخال نتائج المباريات.</p>`;
    return;
  }
...
async function recalcAllPointsInDatabase(){
  const {data:allPreds,error:predErr}=await db.from("predictions_v2").select("*"); 
  const {data:allResults,error:resErr}=await db.from("match_results").select("*");
  ...
  const updates=(allPreds||[]).map(p=>
    db.from("predictions_v2")
      .update({points:calcPoints(p,resultMap.get(String(p.match_id)))})
      .eq("id",p.id)
  );
...
async function saveResultAndScore(){
  ...
  if(isKnockout(m)&&home_goals===away_goals){
    const checked=document.querySelector('input[name="adminQ"]:checked'); 
    if(!checked)return alert("اختر المتأهل الفعلي عند التعادل"); 
    winner_code=checked.value; 
  }
  ...
  await updateKnockoutTeams(match_id,winner_code);
  await recalcAllPointsInDatabase(); 
  await loadAll(); 

  alert("تم حفظ النتيجة وتحديث الترتيب والأدوار تلقائيًا");
}
