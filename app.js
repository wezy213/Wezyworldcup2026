...
function calcPoints(p,r){
  ...
  if(exact){
    ...
    pts += 1;
  }else if(!predDraw && !realDraw && ((ph>pa&&rh>ra)||(pa>ph&&ra>rh))){
    pts += 2;
  }
...
function pointsForPrediction(p){ const r=results.find(x=>String(x.match_id)===String(p.match_id)); return calcPoints(p,r); }
...
if(!predDraw && !realDraw && ((ph>pa&&rh>ra)||(pa>ph&&ra>rh))) return 2;
  if(isKnockout(m)&&predDraw&&realDraw){
    const predictedQualifier=normalizeWinnerCode(p.winner_code,m,r);
    const actualQualifier=normalizeWinnerCode(r.winner_code,m,r);
    if(predictedQualifier && actualQualifier && predictedQualifier!=="DRAW" && predictedQualifier===actualQualifier) pts+=3;
  }
  return pts;
}
